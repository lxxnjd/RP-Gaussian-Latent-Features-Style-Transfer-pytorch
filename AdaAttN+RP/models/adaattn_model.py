...
class AdaAttNModel(BaseModel):
    ...
    def forward(self):
        ...
        print("===== 内容特征（c_feats）各层尺寸 =====")
        for idx, feat in enumerate(self.c_feats):
            print(f"第{idx+1}层特征尺寸: {feat.size()}")
        torch.cuda.synchronize()
        encode_time = encode_start.elapsed_time(encode_end)
        print(f"编码模块（VGG特征提取）耗时: {encode_time:.2f} ms")
        ...
        decode_start.record()  # 开始计时
        self.cs = self.net_decoder(cs, c_adain_feat_3)
        decode_end.record()  # 结束计时
        torch.cuda.synchronize()
        decode_time = decode_start.elapsed_time(decode_end)
        print(f"解码模块（VGG特征提取）耗时: {decode_time:.2f} ms")

        all_time = decode_start.elapsed_time(encode_start)
        print(f"整个风格迁移耗时: {all_time:.2f} ms")
